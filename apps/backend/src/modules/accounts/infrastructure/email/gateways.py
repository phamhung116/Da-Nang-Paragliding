...
class CustomerEmailGateway:
    def send_verification_email(self, context: VerificationEmailContext) -> None:
        subject = "Xác thực email đặt lịch Dù lượn Đà Nẵng"
    ...
    def _build_text_body(self, context: VerificationEmailContext) -> str:
        return (
            f"Xin chào {context.account.full_name},\n\n"
            "Vui lòng mở liên kết dưới đây để xác thực email và đăng nhập Dù lượn Đà Nẵng:\n\n"
            f"{context.verification_url}\n\n"
            f"Liên kết có hiệu lực trong {context.expires_hours} giờ. Nếu bạn không tạo tài khoản, "
            "có thể bỏ qua email này."
        )
...
<html lang="vi">
  <head>
    ...
    <title>Xác thực email Dù lượn Đà Nẵng</title>
  </head>
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#ffffff;border:1px solid #f2d6de;border-radius:28px;overflow:hidden;box-shadow:0 18px 46px rgba(110,26,48,.12);">
            <tr>
              <td style="padding:28px 28px 18px;background:linear-gradient(135deg,#c91842 0%,#8f102b 100%);color:#ffffff;">
                ...
                <h1 style="margin:22px 0 8px;font-size:34px;line-height:1.08;letter-spacing:-.04em;">Xác thực email của bạn</h1>
                <p style="margin:0;font-size:15px;line-height:1.7;color:#ffe7ed;">Hoàn tất bước bảo mật để bắt đầu đặt lịch bay dù lượn tại Đà Nẵng.</p>
              </td>
            </tr>
            <tr>
              <td style="padding:30px 28px 10px;">
                <p style="margin:0 0 14px;font-size:16px;line-height:1.7;">Xin chào <strong>{full_name}</strong>,</p>
                <p style="margin:0 0 22px;font-size:16px;line-height:1.7;color:#6f6267;">Vui lòng bấm nút bên dưới để xác thực email, đăng nhập, đặt lịch và theo dõi hành trình bay.</p>
                <a href="{verification_url}" style="display:inline-block;padding:15px 24px;border-radius:999px;background:#c91842;color:#ffffff;text-decoration:none;font-weight:800;box-shadow:0 14px 28px rgba(201,24,66,.22);">Xác thực email</a>
                <p style="margin:22px 0 0;font-size:14px;line-height:1.7;color:#8b7b82;">Liên kết này có hiệu lực trong <strong>{context.expires_hours} giờ</strong>. Nếu nút không hoạt động, hãy sao chép liên kết sau vào trình duyệt:</p>
            ...
            <tr>
              <td style="padding:18px 28px 30px;">
                ...
                <p style="margin:18px 0 0;font-size:12px;line-height:1.6;color:#9b8d93;">Nếu bạn không tạo tài khoản Dù lượn Đà Nẵng, vui lòng bỏ qua email này.</p>
              </td>
            </tr>
          </table>
        </td>
